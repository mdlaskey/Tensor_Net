...
class GridNet_UB(TensorNet):
    ...
    def __init__(self):
        ...
        self.x = tf.placeholder('float', shape=[None,None, 2])
        self.y_ = tf.placeholder("float", shape=[None,None, 5])
        ...
        self.traj_sum = tf.reduce_mean(-tf.reduce_sum(self.y_ * tf.log(self.y_out), reduction_indices=[2]))

        self.weight_loss = self.traj_sum*self.get_weight(y_out)/self.weights
